...
under_samp_rf<-table(test_balanced_under$HCV_STATUS,pred.rf.under)
...
actual_rf.under<-as.numeric(test_balanced_under$HCV_STATUS)
UNDER_logloss_rf<-logLoss(rf_target_under_IndMat[,2], pred.rf.under.logloss); UNDER_logloss_rf


#4. RANDOM FOREST ALGORITHM FOR UNDER-SAMPLED DATA
rf_both<-randomForest(HCV_STATUS~.,ntree=2,data = data_balanced_both,importance = TRUE)
print(rf_both)

importance(rf_both)
varImpPlot(rf_both,main = "Variable Importance")
plot(rf_both)

pred.rf.both <- predict(rf_both, test_balanced_both, type="prob")
head(pred.rf.both)

#rf_target_both_IndMat<-dummy.data.frame(data=as.data.frame(test_balanced_both$HCV_STATUS), sep="_", verbose=F, dummy.class="ALL")

pred.rf.both<-pred.rf.both[,2]
pred.rf.both.logloss<-pred.rf.both
pred.rf.both[pred.rf.both>=0.5]=1
pred.rf.both[pred.rf.both<0.5]=0
both_samp_rf<-table(test_balanced_both$HCV_STATUS,pred.rf.both)
round(prop.table(table(test_balanced_both$HCV_STATUS,pred.rf.both),1),2)
Both_rf_samp_miscat<-(both_samp_rf[1,2]+both_samp_rf[2,1])/(both_samp_rf[1,2]+both_samp_rf[1,1]+both_samp_rf[2,1]+both_samp_rf[1,2]);Both_rf_samp_miscat
actual_rf.both<-as.numeric(test_balanced_both$HCV_STATUS)
BOTH_logloss_rf<-logLoss(actual_rf.both, pred.rf.under.logloss); UNDER_logloss_rf
