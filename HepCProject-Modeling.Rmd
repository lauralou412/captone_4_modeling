...
test_balanced_over <- ovun.sample(HCV_STATUS ~ ., data = test_patients_vars_imb, method = "over",N = 300000)$data
...
##LOGISTIC MODEL ON MANUAL SAMPLING TECHNIQUES
...
##LOGISTIC MODEL ON OVERSAMPLING TECHNIQUES
...
##LOGISTIC MODEL ON UNDERSAMPLING TECHNIQUES
...
##LOGISTIC MODEL ON BOTH-WAY SAMPLING TECHNIQUES
...
##LOGISTIC MODEL ON ROSE SAMPLING TECHNIQUES
...
data.frame(summary(sampled_log)$coef[summary(sampled_log)$coef[,4] <= .05, 4])
...
both_tree_samp_miscat<-(over_samp_tree[1,2]+over_samp_tree[2,1])/(over_samp_tree[1,2]+over_samp_tree[1,1]+over_samp_tree[2,1]+over_samp_tree[1,2]);both_tree_samp_miscat

## HH still need to get output in terms of probability
over_tree_logloss<-logLoss(as.numeric(test_balanced_over$HCV_STATUS), pred.tree.over)
over_tree_logloss
...
#OVERSAMPLING REGRESSIN TREE MODEL METRICS
...
table(test_balanced_over$HCV_STATUS,pred.tree.over)
round(prop.table(table(test.rose$HCV_STATUS,xp_pred_rose),1),2)
