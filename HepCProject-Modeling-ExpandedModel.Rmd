...
#RANDOM FOREST ALGORITHM FOR OVER- & UNDER-SAMPLED DATA
##LO -  increase the number of trees

data_balanced_both$HCV_STATUS <- as.factor(data_balanced_both$HCV_STATUS)
rf_over_under<-randomForest(HCV_STATUS~.,ntree=20,data = data_balanced_both,importance = TRUE)

#list variables with importance
importance(rf_over_under)
#plot variables by importance
varImpPlot(rf_over_under,main = "Variable Importance")
#plot error decrease with increase in number of trees
plot(rf_over_under)

#predict on the test dataset using the model 
pred.rf.over.under <- predict(rf_over_under, test_balanced_both, type="prob")
head(pred.rf.over.under)

pred.rf.over.under<-pred.rf.over.under[,2]
pred.rf.over.under.logloss<-pred.rf.over.under

#use .5 as cutoff for categorization
pred.rf.over.under[pred.rf.over.under>=0.5]=1
pred.rf.over.under[pred.rf.over.under<0.5]=0
#confusion matrix
over_under_samp_rf<-table(test_balanced_both$HCV_STATUS,pred.rf.over.under);over_under_samp_rf
#confusion proportion table
round(prop.table(table(test_balanced_both$HCV_STATUS,pred.rf.over.under),1),2)
#miscategorization rate
over_under_rf_samp_miscat<-(over_under_samp_rf[1,2]+over_under_samp_rf[2,1])/(over_under_samp_rf[1,2]+over_under_samp_rf[1,1]+over_under_samp_rf[2,1]+over_under_samp_rf[1,2]);over_under_rf_samp_miscat
...
over_under_logloss_rf<-LogLoss(test_balanced_both$HCV_STATUS, pred.rf.over.under.logloss); over_under_logloss_rf
```
...
#0. FOR MANUAL SAMPLING TECHNIQUE
...
pred.xgboost.manual <- matrix(predict(bst, xgbTest), ncol = numClasses, byrow = TRUE)
head(pred.xgboost.both)


## 0. METRICS FOR XGBOOST ALGORITHM FOR MANUAL SAMPLING TECHNIQUE
pred.xgboost.manual<-pred.xgboost.manual[,2]

pred.xgboost.manual.logloss<-pred.xgboost.manual
#use .5 as cutoff for categorization
pred.xgboost.manual[pred.xgboost.manual>=0.5]=1
pred.xgboost.manual[pred.xgboost.manual<0.5]=0
#confusion matrix
manual_samp_xgboost<-table(testY,pred.xgboost.manual);manual_samp_xgboost
#confusion proportion table
round(prop.table(table(testY,pred.xgboost.manual),1),2)
#miscategorization rate
manual_xgboost_samp_miscat<-(manual_samp_xgboost[1,2]+manual_samp_xgboost[2,1])/(manual_samp_xgboost[1,2]+manual_samp_xgboost[1,1]+manual_samp_xgboost[2,1]+manual_samp_xgboost[1,2]);manual_xgboost_samp_miscat
...
manual_logloss_xgboost<-LogLoss(testY, pred.xgboost.manual.logloss); manual_logloss_xgboost

#Variable importance
manual_var_imp<-xgb.importance(colnames(trainY), model = bst)

i=1
while (i<=nrow(manual_var_imp)) {
  index<-as.numeric(manual_var_imp[i,1])
  manual_var_imp[i,1]<-names(trainX[index])
  i<-i+1
  }
  
head(manual_var_imp, n=15)


#1. FOR UNDERSAMPING TECHNIQUE
...
## 1. METRICS FOR XGBOOST ALGORITHM FOR UNDERSAMPLING TECHNIQUE
...
## 2. METRICS FOR XGBOOST ALGORITHM FOR OVERSAMPLING TECHNIQUE
...
## 3. METRICS FOR XGBOOST ALGORITHM FOR BOTH SAMPLING TECHNIQUE
...
#Variable importance
both_var_imp<-xgb.importance(colnames(trainY), model = bst)

i=1
while (i<=nrow(both_var_imp)) {
  index<-as.numeric(both_var_imp[i,1])
  both_var_imp[i,1]<-names(trainX[index])
  i<-i+1
  }
  
head(both_var_imp, n=15)
```
