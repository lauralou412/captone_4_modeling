...
#select expanded variables only
...
man_xgboost_samp_miscat<-(man_samp_xgboost[1,2]+man_samp_xgboost[2,1])/(man_samp_xgboost[1,2]+man_samp_xgboost[1,1]+man_samp_xgboost[2,1]+man_samp_xgboost[1,2]);man_xgboost_samp_miscat
...
### TRY DIFFERENT THRESHOLDS FOR PROBABILITY

pred.xgboost.man <- matrix(predict(bst, xgbTest), ncol = numClasses, byrow = TRUE)
head(pred.xgboost.under)

results_table <- data.frame(Threshold=double(),
                 Miscat_rate=double(),
                 Logloss=double(),
                 false_negatives=double(),
                 false_positives=double())
...
while (i<1){
  ...
  pred.xgboost.man_met<-pred.xgboost.man
  pred.xgboost.man_met[pred.xgboost.man_met>=threshold]=1
  pred.xgboost.man_met[pred.xgboost.man_met<threshold]=0
  #confusion matrix
  man_samp_xgboost_met<-table(testY,pred.xgboost.man_met);man_samp_xgboost_met
  #confusion proportion table
  round(prop.table(table(testY,pred.xgboost.man_met),1),2)
  #miscategorization rate
  man_xgboost_samp_miscat<-(man_samp_xgboost_met[1,2]+man_samp_xgboost_met[2,1])/(man_samp_xgboost_met[1,2]+man_samp_xgboost_met[1,1]+man_samp_xgboost_met[2,1]+man_samp_xgboost_met[1,2]);man_xgboost_samp_miscat

  #calculation logloss
  
  man_logloss_xgboost<-LogLoss(testY, pred.xgboost.man.logloss)

  results_table[threshold*10,1]<-threshold
  results_table[threshold*10,2]<-man_xgboost_samp_miscat
  results_table[threshold*10,3]<-man_logloss_xgboost
  results_table[threshold*10,4]<-man_samp_xgboost_met[1,2]
  results_table[threshold*10,5]<-man_samp_xgboost_met[2,1]
  i<-i+.1
  threshold<-i
}
...
#Variable importance
under_var_imp<-xgb.importance(colnames(trainY), model = bst)

head(under_var_imp)

i=1
while (i<=nrow(under_var_imp)) {
 index<-as.numeric(under_var_imp[i,1])
man_var_imp[i,1]<-names(trainX[index])
i<-i+1
}
